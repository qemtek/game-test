...
def streaks_page():
    ...
    for col in range(12):
        week = []
        for row in range(7):
            d = today - timedelta(days=(11 - col) * 7 + (6 - row))
            ds = d.isoformat()
            ...
            if cnt == 0:
                ...
                level = 'very-high'
            week.append({'date': ds, 'count': cnt, 'level': level})
        heatmap_data.append(week)
...
if __name__ == '__main__':
    database.init_db()
    import os as _os
    app.run(debug=True, port=int(_os.environ.get('FLASK_PORT', 5000)))
